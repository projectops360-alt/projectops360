...
| CAP-040 | Project Import Intelligence | P5 | Implemented | 85% | `lib/import-intelligence` |
...
| CAP-045 | Living Graph Process Mining Layer | P2/P4 | Partial (mining-ready capture + integrity + Isabella read model; pilot-gated) | 28% | `lib/events` (canonical registry/gateway, atomic task/milestone/dependency capture, integrity validator, object-centric refs) + `lib/milestone-flow-ui` + `lib/isabella/process-context`; primary/bulk writers in Roadmap, Execution Map, Delivery, Import, Templates and Phase 0 sync — docs: [baseline](capabilities/CAP-045-process-mining-layer-foundation-baseline.md) · [ontology](capabilities/CAP-045-process-mining-layer-ontology-risk-to-resolution.md) · [platform map](capabilities/CAP-045-process-mining-layer-platform-map-and-validation.md) · [event contract & audit](capabilities/CAP-045-canonical-event-contract-and-source-audit.md) · [Isabella training](isabella-process-mining-layer-training.md) (P2-T3 implemented 2026-07-14; rollout remains project-gated) |
...
| CAP-047 | PMO Process Intelligence Command Center | P1 | Implemented locally — Living Graph interaction parity awaiting UAT (flag OFF) | 95% | `lib/pmo-process-intelligence` ([CAP-047](capabilities/CAP-047-pmo-process-intelligence-command-center.md) — interactive React Flow canvas; organization→stage→project→milestone→activity; live Isabella context; Technical Event Explorer preserved; flag `pmo_process_intelligence_dashboard` OFF) |
| CAP-049 | PMO Simulation Foundation V1 (multivariable what-if) | P1 | **In production** (PR #213, 2026-07-26) | 90% | `lib/pmo-simulation` ([CAP-049](capabilities/CAP-049-pmo-simulation-foundation.md), UX-016 — four intervention kinds over a deterministic engine that composes CPM/EVM/capacity (ADR-012); a scenario never writes to an operational table; units never mix; migración 20260862 aplicada a stage y prod; falta apply-to-project y alcance por proyecto) |
| CAP-050 | Acronym Intelligence (glosario único de métricas) | P2 | **In production** (PR #213, 2026-07-26) | 85% | `lib/acronyms` ([CAP-050](capabilities/CAP-050-acronym-intelligence.md), UX-017 — 49 términos, registro versionado, tres niveles; glosario alcanzable sin datos; convive con `lib/i18n/glossary.ts` que es un linter de paridad, no un glosario; falta cablear tarjetas KPI y panel de nodo) |
