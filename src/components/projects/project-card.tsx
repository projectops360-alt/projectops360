...
export function ProjectCard({ project, statusLabel, href }: ProjectCardProps) {
  ...
  return (
    <Link
      href={href}
      className="group block w-full min-w-0 rounded-xl border border-border bg-card p-5 transition-all hover:border-brand-500/40 hover:shadow-md"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 title={title} className="truncate text-sm font-semibold text-foreground group-hover:text-brand-600 dark:group-hover:text-brand-400">
              {title}
            </h3>
            {description && (
