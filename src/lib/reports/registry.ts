...
export const DATASETS: DatasetDefinition[] = [
  ...
  // 2 — Task Execution
  {
    ...
    columns: [
      ...
      text("owner", "Owner", "Assignment", { description: "Assignee display name — a team member or a named resource. Supports wildcards, e.g. Paul*" }),
      text("owner_id", "Owner ID", "Assignment", { groupable: false, description: "Internal id of the assignee. Use when two people share a display name." }),
      text("discipline", "Discipline", "Assignment", { description: "Type of work — the task's discipline." }),
      text("trade", "Trade", "Assignment", { description: "Type of work — the task's trade." }),
      ...
      num("days_late", "Days Late", "Schedule", { description: "Days past the planned finish: for completed tasks, actual vs planned finish; for open tasks, today vs planned finish. 0 when on time, empty when there is no planned finish." }),
      num("progress_pct", "Progress %", "Schedule", { description: "Reported completion, 0–100." }),
      ...
      num("open_risks", "Open Risks", "Risk", { description: "Count of open or mitigating risks linked to this task." }),
      enumCol("risk_severity", "Top Risk Severity", "Risk", SEVERITY_VALUES, { description: "Highest severity among the task's open risks. Empty when it has none." }),
      num("dependency_count", "Dependencies", "Dependencies", { description: "Number of predecessor tasks this task waits on." }),
      bool("has_dependencies", "Has Dependencies", "Dependencies"),
      num("estimated_hours", "Estimated Hours", "Effort"),
      num("actual_hours", "Actual Hours", "Effort", { description: "Hours from the time log. Only work with no logged time falls back to the value captured by hand." }),
      num("logged_hours", "Logged Hours", "Effort", { description: "The time log alone. 0 when nobody has recorded time against this work yet." }),
      num("hours_variance", "Hours Variance", "Effort", { description: "Actual minus estimated hours. Positive means over the estimate." }),
      num("hours_variance_pct", "Hours Variance %", "Effort", { description: "Hours variance as a percentage of the estimate." }),
    ],
