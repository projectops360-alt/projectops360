...
import {
  ...
  Link2, LayoutGrid, Users2, BarChart3, X, ArrowUpRight,
} from "lucide-react";
import {
      ...
      <div className="grid gap-3 lg:grid-cols-[280px_1fr_330px]">
              ...
              return (
                <button key={String(it.id)} onClick={() => setSelectedId(String(it.id))} className={`w-full rounded-lg border p-2.5 text-left transition-colors ${active ? "border-brand-400 bg-brand-50 dark:border-brand-700 dark:bg-brand-950/30" : "border-border bg-card hover:bg-muted/50"}`}>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-1">
                    ...
                    {it.source === "projectops_scribe" && (
                      <span title={isEs ? "Generado por ProjectOps Scribe" : "Generated by ProjectOps Scribe"} className="inline-flex items-center gap-0.5 rounded bg-brand-100 px-1.5 py-0.5 text-[9px] font-medium text-brand-700 dark:bg-brand-950/40 dark:text-brand-300">
                        <Wand2 className="h-2.5 w-2.5" />Scribe
                      </span>
                    )}
        ...
        {!selected || !f ? (
          ...
          <div className="space-y-3 rounded-xl border border-border bg-card p-4">
            ...
            {/* Source Evidence — provenance for ProjectOps Scribe work items */}
            {selected.source === "projectops_scribe" && (
              <div className="rounded-lg border border-brand-200 bg-brand-50/60 p-3 dark:border-brand-800/60 dark:bg-brand-950/20">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-brand-700 dark:text-brand-300">
                    <Wand2 className="h-3.5 w-3.5" />{isEs ? "Creado por ProjectOps Scribe" : "Created from ProjectOps Scribe"}
                  </span>
                  {selected.source_memory_item_id != null && (
                    <Link href={`/projects/${p.projectId}/memory`} className="inline-flex items-center gap-1 text-[11px] font-medium text-brand-600 hover:underline dark:text-brand-400">
                      {isEs ? "Ver nota de origen" : "Open source note"} <ArrowUpRight className="h-3 w-3" />
                    </Link>
                  )}
                </div>
                {selected.source_reference ? (
                  <p className="text-[11px] italic text-muted-foreground">“{String(selected.source_reference)}”</p>
                ) : (
                  <p className="text-[11px] text-muted-foreground">{isEs ? "Sin extracto de origen." : "No source excerpt."}</p>
                )}
              </div>
            )}
